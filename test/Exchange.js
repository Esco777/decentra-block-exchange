...
describe('Exchange', () => {
  ...
  beforeEach(async () => {
    ...
    token1 = await Token.deploy('Decentra Block', 'DCB', '1000000')
 ...
 describe('Withdrawing Tokens', () => {
    ...
    describe('Failure', () => {
      it('fails for insufficient balance', async () => {
        // Attmept to withdraw tokens without depositing
        await expect(exchange.connect(user1).depositToken(token1.address, amount)).to.be.reverted
      })
    })
 })
 ...
 describe('Making orders', async () => {
    ...
    describe('Failure', async () => {
        it('Rejects with no balance', async () => {
            await expect(exchange.connect(user1).makeOrder(token2.address, tokens(1),token2.address, tokens(1))).to.be.reverted
        })
    })
 })
